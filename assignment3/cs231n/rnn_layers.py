...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    h = np.zeros((N, T, H))
    for t in range(T):
        next_h, tcache = rnn_step_forward(x[:, t, :], prev_h, Wx, Wh, b)
        h[:, t, :] = next_h
        cache.append(tcache)
        prev_h = next_h
    ##############################################################################
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    cache = (x, Wx, Wh, next_c, prev_c, prev_h, igate, fgate, ogate, ggate)
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    #############################################################################
    N, H = dnext_h.shape
    x, Wx, Wh, next_c, prev_c, prev_h, igate, fgate, ogate, ggate = cache
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        next_h, next_c, tcache = lstm_step_forward(
